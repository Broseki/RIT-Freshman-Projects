...
A program that draws two squares of a given side length.  If the program is run
as:
...
authors: Sean Strout @ RIT CS & Michael Canning @ RIT CS
"""
...
def draw_second_square(length : int) -> None:
    """
    Draws a second square of a given length to the left of the first.
    :param length: the length in pixels of one side of the square
    :return: None
    """
    turtle.left(180)
...
def main() -> None:
    ...
    draw_square(length)
    print("Drawing second square with the same dimensions as the first.")
    draw_second_square(length)
